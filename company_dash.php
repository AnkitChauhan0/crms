	...
	<head>
		...
		<script>
			...
			function changeStatus(val,app_id,object){
					...
					_xmlhttp.open("GET","updateStatus.php?s="+val+"&app_id="+app_id,true);
					xmlhttp.send();
		...
		<h3>COMPANY DASHBOARD</h3>
			<div class="row">
				<div class="col-sm-3" style= " margin:10px; " >
					<div class="well row" style="background:rgba(0,0,0,0.4); border-radius: 30px;height:auto; ">
					...
					<?php
						...
						echo"  <div class=\"table-responsive table-bordered\" style=\"background: rgba(0,0,0,0.4);\">            
                ...
                color: black;
						...
						echo"	<div class=\"table-responsive table-bordered\" >            
						 					  <table class=\"table table-hover\" style=\"background: rgba(0,0,0,0.4); border-radius: 30px;\">
											  <tr>
											  <th> Job Id </th>
											   <th>Job Title</th>
											   <th>Salary</th>
										...
										while($row = $result->fetch_assoc()){	   
												...
												echo		   "
											   <tr>
											   <td> <input  type=\"number\" name=\"job_id\" value=\"".$row['job_id']."\" readonly maxlength=\"6\" size=\"6\" style=\"width:40px; background-color: black; color:white;\"></td>
											   <td>".$row['job_title']."</td>
											   <td>".$row['salary']."</td>
											   <td>".$row['location']."";
												echo " <button type=\"submit\" style=\"width:20px; background-color: black; color:white;\">X</button> </td>";
											   echo "</tr>";
											   echo "</form>";
										}	
						...
						?>
						<a href="vacancy.php" style="color : white ">CREATE VACANCY</a>
					</div>
				</div>
				<div class="well col-sm-4" style="background:rgba(0,0,0,0.4); border-radius: 30px; height:auto; margin:10px;">
					...
					<div id="display">
						<img class="img-responsive " src="CSS/Image/c1.jpg" height="120px" width="120px" align="center" style="border-radius:50%"></img>
						<div class="table-responsive table-bordered" style="background: rgba(0,0,0,0.4);" >            
							<table class="table table-hover">
							  <tr>
								<th>Name</th>
								<td>---</td>
							...
							<hr>
				<input type="checkbox" style="margin-left: 170px;" data-toggle="toggle" data-on="Accepted" data-off="Rejected"></td>
						</div>
					</div>
					<div id="dispjob">
					</div>
				...
				<div class="well col-sm-4" style="background:rgba(0,0,0,0.4); border-radius: 30px; height:auto; margin:10px;">
					<h3>NAME OF STUDENTS APPLIED!</h3>
					<p style="color:white;">Click for more details about the student</p>
					<div id="sapp">
					<?php
					 $servername="localhost";
					 $username="root";
					...
					echo  "<div class=\"table-responsive table-bordered\" style=\"background: rgba(0,0,0,0.4);\" >            
					  <table class=\"table table-hover\">
						...
						while($row=$result->fetch_assoc()){
						   ...
						   $sql2="Select * from student where email='".$row['s_mail']."'";
						   $result2=$conn->query($sql2);
						   $row2=$result2->fetch_assoc();
						  // $ran = mt_rand(10,999999); 
						   echo "<form action=\"updateStatus.php\" method=\"POST\">";
						   echo "<tr onclick=\"trclick('".$row['s_mail']."','".$row['job_id']."',this)\">";
						   echo "<td> <input  type=\"number\" name=\"app_id\" value=\"".$row['app_id']."\" readonly maxlength=\"8\" size=\"8\" style=\"width:78px; background-color:black;\"></td>";
						   echo "<td>".$row2['name']."</td>";
						   echo "<td>".$row['job_id']."</td>";
						   echo "<td>";
						    
							echo "<select name=\"status\"style=background-color:black;\">";
							if($row['status'] == 0){
							...
							echo " <button type=\"submit\ style=\"background-color:black;\">Update</button> </td>";
						   echo "</tr>";
						   echo "</form>";
						   }
